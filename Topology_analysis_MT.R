...
setwd("/Users/esforsythe/Documents/Beilstiein_lab_research/BIOINFORMATICS/Brassicaceae_Phylo/0_170207_REDO_w_Chir/170320_Mitochondria/RAxML_170416")
...
trees<-read.tree("MT_trees_catfile170416")
Ntrees<-length(trees)

#Quick function to plot trees
PlotTreesFunc<-function(tree){
  #tree<-compute.brlen(tree, 100)
  tips<-tree$tip.label
  Brap_tip<-grep("Br_", tips)
  root_tree<-root(tree, Brap_tip, resolve.root=TRUE)
  root_tree$node.label=tree$node.label
  plot.phylo(root_tree, cex=0.7)
  #nodelabels(root_tree$node.label, cex=0.7)
}

#Apply the quick function
lapply(trees, PlotTreesFunc)

#Make function for topology analysis
TopAnalFunc<-function(tree){
  
  ###########################################################################################
  ############                    Root the tree by the Brap tip                  ############
  ###########################################################################################
  ...
  tips<-tree$tip.label
  Brap_tip<-grep("Br_", tips)
  root_tree<-root(tree, Brap_tip, resolve.root=TRUE, edgelabel=TRUE)
...
#Apply the topology analysis function to all trees
...
plot(BC_hist, main="distibution of BS scores (BC topology)", xlab= "BS score", border="red", xlim=c(0,100), ylim=c(0,2))
plot(AC_hist, main="distibution of BS scores (AC topology)", xlab= "BS score", border="dark green", xlim=c(0,100), ylim=c(0,2))
plot(AB_hist, main="distibution of BS scores (AB topology)", xlab= "BS score", border="blue", xlim=c(0,100), ylim=c(0,2))


#export a CSV file
write.csv(output_df, file = "MT_top_analysis_results170416")