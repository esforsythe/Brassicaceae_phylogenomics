...
#Calculate the Dstat
Dstat_observed<-(BC_count - AB_count)/(BC_count + AB_count)
...
Boot_Dstat <- function(X=split_d){
  ...
  AB_count<-table["AB_topology"]
  BC_count<-table["BC_topology"]
  Dstat_temp<-(BC_count - AB_count)/(BC_count + AB_count)
...
out_full<-c("All", BC_count, AB_count, Dstat_observed, SD, z, P_value)
...
z<-(Dstat_observed-0)/SD
P_value<-2*pnorm(-abs(z))
out_chrom1<-c("Chr1", BC_count, AB_count, Dstat_observed, SD, z, P_value)
...
z<-(Dstat_observed-0)/SD
P_value<-2*pnorm(-abs(z))
out_chrom2<-c("Chr2", BC_count, AB_count, Dstat_observed, SD, z, P_value)
...
z<-(Dstat_observed-0)/SD
P_value<-2*pnorm(-abs(z))
out_chrom3<-c("Chr3", BC_count, AB_count, Dstat_observed, SD, z, P_value)
...
z<-(Dstat_observed-0)/SD
P_value<-2*pnorm(-abs(z))
out_chrom4<-c("Chr4", BC_count, AB_count, Dstat_observed, SD, z, P_value)
...
z<-(Dstat_observed-0)/SD
P_value<-2*pnorm(-abs(z))
out_chrom5<-c("Chr5", BC_count, AB_count, Dstat_observed, SD, z, P_value)
...
z<-(Dstat_observed-0)/SD
P_value<-2*pnorm(-abs(z))
out_chrom6<-c("Chr6", BC_count, AB_count, Dstat_observed, SD, z, P_value)
...
z<-(Dstat_observed-0)/SD
P_value<-2*pnorm(-abs(z))
out_chrom7<-c("Chr7", BC_count, AB_count, Dstat_observed, SD, z, P_value)
...
z<-(Dstat_observed-0)/SD
P_value<-2*pnorm(-abs(z))
out_chrom8<-c("Chr8", BC_count, AB_count, Dstat_observed, SD, z, P_value)
...
names(cat_out_df) <- c("Chromosome", "BC_trees", "AB_trees", "Dstat_obs", "SD", "Z_score", "P_value")
#rownames(cat_out_df) <- c("All", "Chr1", "Chr2", "Chr3", "Chr4", "Chr5", "Chr6", "Chr7", "Chr8")

#write.csv(cat_out_df, file = "JB_full_ACsp_170913.csv")
#write.csv(cat_out_df, file = "JB_CSC_ACsp_170913.csv")
...
#pdf(file="Jack_AC_170626.pdf",width=6,height=4)

#command for printing pie to pdf TOGGLE FOR CSC
pdf(file="Jack_AC_170626_CSC.pdf",width=6,height=4)
...
ggplot(cat_out_df, aes(x=cat_out_df$Chromosome, y=Dstat)) + geom_point() + geom_errorbar(width=.1, aes(ymin=ymin, ymax=ymax)) + 
  ...
  geom_point(shape=21, size=c(7, 3, 3, 3, 3, 3, 3, 3, 3), fill=c("dark gray", "dark gray", "dark gray", "dark gray", "dark gray", "dark gray", "dark gray", "dark gray", "dark gray")) + 
  #ylim(-0.1, 0.7) + 
  xlab("Chromosome") + 
...
#write.csv(cat_out_df, file = "Boot_full_ACsp_170913.csv")
#write.csv(cat_out_df, file = "Boot_CSC_ACsp_170913.csv")

#Set minimum and maximum values for error bars
ymin<-as.numeric(paste(cat_out_df$Dstat_obs))-as.numeric(paste(cat_out_df$SD))
ymax<-as.numeric(paste(cat_out_df$Dstat_obs))+as.numeric(paste(cat_out_df$SD))
Dstat<-as.numeric(paste(cat_out_df$Dstat_obs))

#command for printing pie chart to pdf
#pdf(file="Boot_AC_170626.pdf",width=6,height=4)
#command for printing pie to pdf TOGGLE FOR CSC
pdf(file="Boot_AC_170626_CSC.pdf",width=6,height=4)
...
write.csv(cat_out_df, file = "boot_out_ACtree170806.csv")
